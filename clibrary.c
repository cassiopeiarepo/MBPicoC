...
void BME_init(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
  ...
  if (!status) sendContent("Could not find a valid BME280 sensor, check wiring!");  
  delay(200);
  bme.readTemperature();
  bme.readPressure();
  bme.readHumidity();
...
void TFT_Gauge_draw(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
  ...
  else Serial.printf("GaugeNumber %d is not initialized\n",gaugeNumber);
}
...
{  
  ...
  else Serial.printf("VbarGauge %d is not initialized\n",gaugeNumber);
}
...
{  
  ...
  else Serial.printf("HbarGauge %d is not initialized\n",gaugeNumber);
}
...
void iconsoleOut(char * what)
{
 ...
 // DEBUGP.printf("Bufp in iConsole->%s\n",Bufp);
  if (strstr(Bufp,">-> ")>0){ Bufp=strstr(Bufp,">->")+4;//DEBUGP.print("d >-> "); 
 // DEBUGP.println(Bufp);
  }
  if (Bufp[strlen(Bufp)-1]=='\n') { // DEBUGP.print("has [CR]"); 
    hasCR=1; Bufp[strlen(Bufp)-1]=0;  
  }
  if (strstr(Bufp,"</td></tr>")>0) { *strstr(Bufp,"</td></tr>")=0; // DEBUGP.print("d </td></tr");
  // DEBUGP.println(Bufp); 
  }
  ...
  if (strlen(Bufp) > 0) { tft.print(Bufp); if (toScroll==0) cCount+=strlen(Bufp); // DEBUGP.printf("tft.print(\"%s\")\n",Bufp);
  }
  
  if (hasCR==1) { iScroll(); tft.setCursor(0,240-slines); // DEBUGP.println("Ordered scroll for [CR]"); 
  cCount=0;
    }
}
...
void Watch(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
  int i;
  char * what=(char *)Param[0]->Val->Pointer;
  for (i=0;i<nwatches;i++) if (strcmp((char *)&watches[i][0],what)==0) { Ssend("Watch already in place"); return; } 
...
void PrintType(struct ValueType *Typ)
{
    ...
    switch (Typ->Base)
    {
        ...
        case TypeArray:         PrintType(Typ->FromType); PrintAndWebOut(F("[")); if (Typ->ArraySize != 0) {snprintf((char *)&b,20,"%d",Typ->ArraySize);
  ...
  {
      char lb[3];
      snprintf((char *)&lb,2,"%c",OutCh);lb[1]=0;sprint((char *)&lb);
  }
    ...
    void PrintFP(double Num)
    {
        ...
        { 
            for (MaxDecimal = 6; MaxDecimal > 0 && abs(Num) >= 1e-7; Num = (Num - (long)(Num + 1e-7)) * 10, MaxDecimal--)
                PrintCh(z);
            char lbuf[21];
            snprintf((char *)&lbuf,20,"%f",Num+1e-7);PrintAndWebOut((char *)&lbuf);
        }
        else
            PrintCh(z);
  ...
  void PrintWithCommas(unsigned long n,int width)
  {
    ...
    for (i=wd;i<width;i++) sendContent(" ");
    for (i=0;i<4;i++) {m[i]=n%1000;n=n/1000;}
    if (nh<1000) snprintf((char *)&Bf,200,"%d",m[0]);
    else if (nh<1000000) snprintf((char *)&Bf,200,"%d,%03d",m[1],m[0]);
    else if (nh<1000000000) snprintf((char *)&Bf,200,"%d,%03d,%03d",m[2],m[1],m[0]);
    else snprintf((char *)&Bf,200,"%d,%03d,%03d,%03d",m[3],m[2],m[1],m[0]);
   ...
   literal=0;
    ...
    for (FPos = Format; *FPos != '\0'; FPos++)
    {
      ...
      if (*FPos == '%')
      {
        ...
        #endif // NO_FP
          case '%': { Buf[--ip]=0;ip=0;send((char *)&Buf); strcpy((char *)&Buf,""); FormatType = NULL; break; }
            ...
            else
            {
              ...
              //  printf("doing int with format %s and num %d\n",(char *)&Buf,ExpressionCoerceInteger(NextArg));
                ...
                {  // handle the new , format type
                  ...
                  {
                    *cp=0;
                    snprintf((char *)&Bf,200,"%s",(char *)&Buf);sendContent((char *)&Bf);cp++;
                  }
  ...
  //      Serial.println("Got to format specifier");
        ...
        switch (*FPos)
        {
          ...
          case 'f': FormatType = &FPType; break;
    #endif // NO_FP
          case '%': { Buf[--ip]=0;ip=0;strncat(oBuf,(char *)&Buf,500); strcpy((char *)&Buf,""); FormatType = NULL; break; }
          case '\0': FPos--; FormatType = NULL; break;
            ...
            else
            {
              ...
              //  printf("doing int with format %s and num %d\n",(char *)&Buf,ExpressionCoerceInteger(NextArg));
                ...
                {  // handle the new , format type
                  ...
                  cp=strstr((char *)&Buf,"%");
                  *cp=0;
                  snprintf((char *)&Bf,200,"%s",(char *)&Buf);strncat(oBuf,(char *)&Bf,500);cp++;
                  int wd=0;while (isdigit((int)*cp)) wd=wd*10+*cp++ - '0';
                  PrintWithCommas((unsigned long)ExpressionCoerceUnsignedInteger(NextArg),wd); break;
                }
    #ifndef NO_FP
 ...
 void Libstrncat(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
  ...
  { 
    ...
    if (fontIndex<1 || fontIndex>3) Serial.printf("Only font sizes of 1-3 allowed\n");
...
void  oconsoleInit(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
  {
    for (int i=0;i<6;i++) strcpy((char *)&ochars[i][0],"");
    display.clear();
    display.display();
    OLEDdidWhen=millis();
  }
...
{ 
  ...
  sendContent(FileName);sendContent("\", ...)\n");
  showNums=Param[1]->Val->Integer;  
#ifdef ESP32
  File fin=SPIFFS.open((char *)&fname,"r");if (!fin) {
#else
  fs::File fin=SPIFFS.open((char *)&fname,"r");if (!fin) {
#endif
  
          sendContent("file open failed for ");sendContent((char *)&fname);
          buf[0]=0;
          return;
      }     
...
void Ctest (struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) 
{
    ...
    sendContent((char *)&bf);
    Ssend((char *)&bf);
    for (int i=3;i<11;i++) { snprintf((char *)&bf,20,"test line %d\n",i);sendContent((char *)&bf);Ssend((char *)&bf); }
    ...
    if (ps!=0)
    {
      ...
      *ps++=10;cp=ps;
      if (*cp==0) {
        ...
        toConsole=0;
        if (didPlines==0) { literal=1;sendContent("</textarea><br/>Global Level Outputs<br/><table class=\"text2\" frame=\"vsides\"  rules=\"cols\" cellpadding=\"2\">");}
...
//printf("%s %d\n",filename,lineno);
  ...
  char lbuf[20];
  sendContent("Breakpoint set at ");sprintf((char *)&lbuf,"%d",tempParse.Line);sendContent((char *)&lbuf); sendContent(" in ");
...
//printf("%s %d\n",filename,lineno);
  ...
  tempParse.CharacterPos=0;
  char lbuf[20];
  sendContent("Breakpoint cleared at ");sprintf((char *)&lbuf,"d",tempParse.Line);sendContent((char *)&lbuf); sendContent(" in ");
  sprintf((char *)&lbuf,"s\n",tempParse.FileName);sendContent((char *)&lbuf);
...
void SDB(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
   ...
   if (dmode==1) Prompt="debug> ";
   else Prompt="picoc> ";
sendContent(Prompt);
   Parser->DebugMode=dmode;
   char lbuf[20];sprintf((char *)&lbuf,"%d",BreakpointCount);
  sendContent((char *)&lbuf);sendContent(" Breakpoints are set\n");
...
void isprime(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
  ...
  int t1;long istrt=n;
  long lastp=n;
  sendContent("\nFinding primes between ");PrintWithCommas(n,0);sendContent(" & ");
  PrintWithCommas(lim,0);sendContent("\n\n");  
  t1=millis();
  while (n < (lim)) 
  {
  k = 3;
  ...
  while ((k * k <= n) && p) 
    ...
    if (p) 
    {
      ...
      if ((pc %interval) ==0)
      {
        sendContent("prime # ");PrintWithCommas(pc,6);sendContent(" is ");PrintWithCommas(n,0);
        char lbuf[20];
        sprintf((char *)&lbuf,"%5.2f %% are prime\n",interval*100.0/(n-lastp));sendContent((char *)&lbuf); 
        lastp=n;
      }
    }  
  }
  float secs=(millis()-t1)/1000.0;
  sendContent("Last prime was ");PrintWithCommas(phold,0);sendContent("\n");
  sendContent("\n\n");PrintWithCommas(pc,0);sendContent(" primes found\n");
  PrintWithCommas(mcount,0);sendContent(" prime check loops performed in ");
  sprintf((char *)&lbuf,"%0.2f",secs);sendContent((char *)&lbuf);sendContent(" sec\n");       
  sendContent("That's ");PrintWithCommas(mcount/secs,0);sendContent(" per second\n");
...
int checkPin(int pin)
{
  if ((pin == 2) || //1
        ...
        (pin == 32) || (pin == 33)) //2
        return 1;
  else Serial.printf("pin %d is not available for PWM\n",pin);
}
char servoReport=1;
...
void setServoReport(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
  ...
  {
    sendContent("\nfree heap size = ");char lbuf[20];sprintf((char *)&lbuf,"%d\n",system_get_free_heap_size());sendContent((char *)&lbuf);
  }
#endif
...
void PinMode (struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) 
{
  ...
  if (pin>39)  {sendContent("Invalid pin ");sprintf((char *)&b,"%d",pin);sendContent((char *)&b);sendContent(" specified!\n"); return; }
  val = (char *)Param[1]->Val->Pointer; 
      if (strcmp((char *)val,"OUTPUT")==0) {pinMode(pin,OUTPUT); }
      else if (strcmp((char *)val,"INPUT")==0) {pinMode(pin,INPUT); }
      else if (strcmp((char *)val,"INPUT_PULLUP")==0) {pinMode(pin,INPUT_PULLUP);} 
        else {sprintf((char *)&b,"Mode of %s not Recognized\n",(char *)val);sendContent((char *)&b);}
}
void DigitalWrite (struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs) 
{
  ...
  if (pin>39 ) {PrintAndWebOut("Invalid pin ");sprintf((char *)&b,"%d",pin);PrintAndWebOut((char *)&b);PrintAndWebOut(" specified!\n"); return; }
...
void myReadFile(char * FileName)
{
  ...
  fs::File fin=SPIFFS.open((char *)&fname,"r");
  if (!fin )
        {PrintAndWebOut("can't read file ");sendln((char *)&fname);return;}
    fin.read((uint8_t *)&buf,BUFSIZE-1);
    fin.close();
}
char buff[8];
